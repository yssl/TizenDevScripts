...
for fileName in os.listdir(outDir):
    if not os.path.isdir(outDir+fileName):
        rawLogFileNames.append(fileName)
        print fileName

summaryPath = '%s/summary.txt'%(outDir)
labelSet = []
valueSet = []

# read & sum
for f in range(len(rawLogFileNames)):
    fileName = rawLogFileNames[f]
    labels = []
    values = []
    with open(outDir+fileName, 'r') as file:
        ...
        for line in lines:
            if '# Command:' in line:
                ...
                labels.append('')
                labels.append(line)
                labels.append('# Number of data: %d'%len(rawLogFileNames))
